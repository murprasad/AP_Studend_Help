...
function courseFile(dataDir, course) {
 ...
 */
export function recordGateOutcome(dataDir, course, topic, gateResult) {
    ...
    if (gateResult.ok) {
        ...
        const existing = entry.failures[gate] ?? {
            gate,
            count: 0,
            lastSeen: now,
        };
        ...
        if (gateResult.reason)
            existing.lastReason = gateResult.reason.slice(0, 200);
 ...
 */
export function getNegativePromptForTopic(dataDir, course, topic, topN = 5) {
    const file = loadCourse(dataDir, course);
    const entry = file.topics[topic];
    if (!entry || Object.keys(entry.failures).length === 0)
        return "";
...
export function getTopicStats(dataDir, course, topic) {
    const file = loadCourse(dataDir, course);
