...
import type { Template, TopicScores, TemplateScore } from "./types.js";
...
function scoresFile(dataDir: string, course: string): string {
  return join(dataDir, "template-scores", `${course}.json`);
}

interface CourseScoresFile {
  course: string;
  topics: Record<string, TopicScores>;
  updatedAt: string;
}

function loadCourse(dataDir: string, course: string): CourseScoresFile {
  const path = scoresFile(dataDir, course);
  if (!existsSync(path)) {
    return { course, topics: {}, updatedAt: new Date().toISOString() };
  }
  try {
    const parsed = JSON.parse(readFileSync(path, "utf-8")) as CourseScoresFile;
...
function saveCourse(dataDir: string, file: CourseScoresFile): void {
...
export function scoreTemplate(
  ...
  topic: string,
  templateId: string,
  outcome: "passed" | "failed-after-retries",
): void {
  ...
  let entry: TemplateScore = topicScores.templates[templateId] ?? {
    templateId,
    attempts: 0,
    passed: 0,
    passRate: 0,
