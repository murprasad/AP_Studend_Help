 ...
 */
function parseQuestionFromLlm(raw: string, course: string, topic: string): Question | null {
  ...
  if (Array.isArray(parsed.questions) && parsed.questions.length > 0) {
    Object.assign(root, parsed.questions[0] as Record<string, unknown>);
  }
  if (typeof root.questionText !== "string") return null;
  if (!Array.isArray(root.options)) return null;
  if (typeof root.correctAnswer !== "string") return null;
  if (typeof root.explanation !== "string") return null;
  return {
    ...
    difficulty: (root.difficulty as Question["difficulty"]) ?? "MEDIUM",
    course,
  };
}
 ...
 */
function defaultBuildUserPrompt(
  course: string,
  topic: string,
  ...
  if (spec && typeof spec === "object") {
    ...
    const useful = ["topic_weights", "difficulty_mix", "function_types_in_scope"] as const;
    const lite: Record<string, unknown> = {};
    for (const k of useful) if (k in spec) lite[k] = spec[k];
  ...
  if (previousFailure && !previousFailure.ok) {
    parts.push(
      `Previous attempt FAILED gate "${previousFailure.gate}": ${previousFailure.reason ?? "(no reason)"}.`,
      `Re-generate the question fixing that specific failure. Do not repeat it.`,
    );
  }
 ...
 */
export async function generateWithFeedback(input: GenerateInput): Promise<LoopResult> {
  const {
    course,
    topic,
    spec,
    ...
    forceTemplateId,
    buildUserPrompt: customBuildUserPrompt,
  } = input;
  ...
  const gateHistory: GateOutcome[] = [];
  let previousFailure: GateOutcome | null = null;
