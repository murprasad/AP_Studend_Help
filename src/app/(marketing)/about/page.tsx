    ...
    <div className="max-w-3xl mx-auto px-4 py-16 space-y-16 text-center">
      ...
      <div className="space-y-4">
        <div className="flex items-center justify-center gap-3">
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            <Badge className="bg-blue-500/20 text-blue-800 dark:text-blue-400 border-blue-500/30 text-xs font-semibold">Beta 11.0</Badge>
          </div>
        </div>
      ...
      <div className="space-y-6">
        <div className="text-center space-y-1">
          ...
          <h2 className="text-xl font-bold">What&apos;s New in Beta 11.0</h2>
          <p className="text-sm text-muted-foreground">A landing-page rebuild driven by real student feedback (&ldquo;feels like another AI study tool&rdquo;) plus a complete restructure of how courses are gated. Hero now demos a real wrong-answer flow on its own. CLEP/DSST handed off to PrepLion. Course visibility is now a per-bank quality gate (≥200 vetted questions before users see it). And a permanent E2E guard blocks &ldquo;Something hiccupped&rdquo; React errors from ever shipping again.</p>
        </div>
        <div className="grid sm:grid-cols-2 gap-3 text-left">
          {[
            { icon: Sparkles, color: "text-blue-500", bg: "bg-blue-500/10", title: "Hero auto-shows the failure flow", desc: "Hero replaced static analytics screenshot with a live interactive question. Visitor doesn&apos;t even click — at 2.5s, the demo auto-clicks a wrong answer so they see the tension banner (&ldquo;You&apos;re confusing media history with political diffusion&rdquo;). Pause at first user click. Designed to flip the demo from &ldquo;static product showcase&rdquo; to &ldquo;live mistake experience&rdquo; in 5 seconds." },
            { icon: Brain, color: "text-blue-500", bg: "bg-blue-500/10", title: "Quality gate: only courses with ≥200 vetted Qs are visible", desc: "Earlier we shipped questions with arithmetic errors and answer-key bugs. Students were marked wrong on questions where they reasoned correctly. Now, courses live behind a SiteSetting allowlist. A course shows up only after passing 7 deterministic gates (math, structural, distractor-leak, stimulus, source-attribution, figure, course-contracts) plus two cross-family LLM judges (Gemini + GPT-4o). 13 courses live, 16+ rebuilding to threshold autonomously." },
            { icon: Heart, color: "text-blue-500", bg: "bg-blue-500/10", title: "CLEP &amp; DSST → PrepLion", desc: "StudentNest serves AP/SAT/ACT only. CLEP and DSST seekers were trying to use marketing copy that didn&apos;t reflect us. Permanent 308 redirects on /clep-prep, /dsst-prep → preplion.ai. Footer carries a small &ldquo;CLEP &amp; DSST? → PrepLion&rdquo; handoff link for honest cross-product navigation." },
            { icon: ShieldCheck, color: "text-blue-500", bg: "bg-blue-500/10", title: "Permanent runtime-error guard", desc: "Beta 11.0 hotfix: a React hook-order bug in SageChat crashed the landing page on scroll (&ldquo;Something hiccupped&rdquo;). Existing tests asserted text visibility but never simulated user scroll. New permanent E2E spec walks the page like a user — load, scroll, click — and hard-fails on any uncaught pageError, React #310 console signature, error-boundary text, or same-origin 500. Runs on every staging gate. The class of bug that shipped Beta 11.0&apos;s crash is now blocked." },
            { icon: Globe, color: "text-blue-500", bg: "bg-blue-500/10", title: "Sidebar: AP + SAT + ACT all visible together", desc: "Test user reported only AP courses showing in their sidebar, even though SAT/ACT were also vetted. Root cause: legacy track-lock (one signup track = one course family). Fixed: students prep for multiple exams; sidebar now shows all 3 group tabs regardless of signup track. Track only determines the default-selected tab, not a hard lock. FRQ Practice nav gated by current course family, not signup track." },
            { icon: Calendar, color: "text-blue-500", bg: "bg-blue-500/10", title: "Day 1 → Day 7 timeline + multi-Q tension section", desc: "Replaced 8 generic feature cards with a concrete time-to-readiness flow: Day 1 take diagnostic, Day 2 weak units surface, Day 3 practice only what matters, Day 5 timed simulation, Day 7 ready. Plus a &ldquo;Most students think they&apos;re ready / Until this happens&rdquo; section showing 5 questions, 3 wrong, confidence drops to 40%. Slightly uncomfortable on purpose — catches overconfidence early." },
          ].map(({ icon: Icon, color, bg, title, desc }) => (
