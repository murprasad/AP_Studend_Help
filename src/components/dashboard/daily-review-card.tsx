...
import { RotateCcw, Loader2, CheckCircle, XCircle, ChevronRight } from "lucide-react";
...
export function DailyReviewCard({ course }: Props) {
  ...
  function handleAnswer(letter: string, correctAnswer: string) {
  ...
  // Extract correct answer letter from options (first char)
  // We don't have correctAnswer on the question object from this endpoint
  // So just show the question; for full answer checking we'd need a session
  // For now, show the review card as a "read and recall" mode
  return (
    <Card className="card-glow border-emerald-500/20">
      <CardHeader>
      ...
      <CardContent className="space-y-3">
        <p className="text-sm font-medium leading-relaxed line-clamp-3">{q.questionText}</p>
        {options.length > 0 ? (
            ...
            {options.map((opt, i) => {
              ...
              return (
                <button
                  key={i}
                  onClick={() => next()}
                  className="w-full text-left px-3 py-2 rounded-lg border border-border/40 hover:bg-accent text-sm transition-all"
                >
                  {opt}
                </button>
              );
          ...
          <Button size="sm" onClick={next} className="w-full gap-1">
            Next <ChevronRight className="h-3.5 w-3.5" />
          </Button>
        )}
        <p className="text-xs text-muted-foreground text-center">{q.topic}</p>
      </CardContent>
    </Card>
  );
}
