...
/**
 * Pass Ready Cert — shareable artifact when predicted readiness is high.
 *
 * This is a post-result trust surface. It should read like a student-facing
 * exam artifact, not an internal analytics export.
 */
...
function courseFamily(course: string): "SAT" | "ACT" | "CLEP" | "DSST" | "exam" {
...
function calibrationText(family: ReturnType<typeof courseFamily>): string {
  switch (family) {
    case "SAT":
      return "Predicted SAT score is calibrated to SAT scoring";
    case "ACT":
      return "Predicted ACT score is calibrated to ACT scoring";
    case "CLEP":
      return "Predicted CLEP score is calibrated to CLEP scoring";
    case "DSST":
      return "Predicted DSST score is calibrated to DSST scoring";
    default:
      return "Predicted score is calibrated to exam scoring";
  }
}
...
export function PassReadyCert({
  ...
  const calibration = calibrationText(family);

  async function handleShare() {
    const text = `I'm at ${predictedPassPercent}% readiness on ${courseDisplayName} after ${studyDays} days using PrepLion.`;
    const url = window.location.origin;

    if (navigator.share) {
      try {
        await navigator.share({ title: "PrepLion — Exam Ready", text, url });
  ...
  function handleDownload() {
    ...
    w.document.write(`
      <html>
      <head><title>PrepLion Exam Ready — ${studentName}</title></head>
      <body style="font-family: -apple-system, system-ui, sans-serif; padding: 40px; background: #fafaf9;">
        <div style="max-width: 600px; margin: 0 auto; padding: 48px; border: 2px solid #10b981; border-radius: 24px; background: linear-gradient(135deg, rgba(16,185,129,0.05), rgba(245,158,11,0.05));">
          <div style="text-align: center;">
            ...
            <h1 style="font-size: 24px; font-weight: 700; margin: 16px 0 8px;">PrepLion — Exam Ready</h1>
            ...
            <p style="color: #9ca3af; font-size: 11px; margin: 0;">${window.location.hostname} · ${calibration}</p>
          </div>
        </div>
