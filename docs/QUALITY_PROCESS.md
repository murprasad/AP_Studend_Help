...
### Mechanical enforcement (D16, 2026-06-12) — this is no longer policy-only
The rules above kept getting skipped because they were *advisory* (dropped at compaction; the author self-attested). They are now wired into `scripts/pre-release-check.js` as HARD gates that run inside `pages:deploy`:
- **G4 provenance:** the QA-walk artifact is rejected as **SELF-AUTHORED** unless an independent `Agent`/subagent transcript (`~/.claude/projects/<cwd-mangled>/<session>/subagents/agent-*.jsonl`) **postdates the newest src change**. A hand-typed walk `.md` cannot pass — REV + the student walk MUST be real Agent invocations (separate context).
- **G5 team sign-off:** the newest release manifest must **postdate src** AND its front-matter must show **G1 PO / G2 DEV / G3 REV / G4 QA / G5 RM = `approved`** (G6 SRE may stay `pending` until post-deploy) AND cite the fresh `qa_walk_log`. You cannot ship with a role still `pending`.
- **Workflow note:** make ALL src edits first, then run the final independent walk + write the manifest LAST, so both postdate the tree. Editing src again re-fails the gate by design. See Defect Ledger **D16**.
...
- 2026-06-11 · D9 journey leads with JUDGMENT (forced diagnostic + 0/N + projected-score) though items 0–2 marked "✅ live" · item flipped to ✅ from partial code-ship, no clause-conformance + no QA persona walk · G1/G4 · warm-up→done direct + diagnostic opt-in + score-verdict removed; **no goal-doc item flips to ✅ without G1 conformance-agent + G4 QA-walk** (BIQ_LEAD_WITH_LEARNING_DEFECTS_2026-06-11.md). PL-found; SN journey parity audit pending.
- 2026-06-11 · D10 broken duplicate-answer-SET question served (PL College Algebra; (D)"-5,3"≡(E)"3,-5") · dup gate string-compared order-sensitively + normalizer never stripped prefix (D11) · G2/G3 · new `options-duplicate-set` order-independent gate w/ ordering-question guard. SN: mirror gate + sweep pool.
- 2026-06-11 · D11 normalizeOptForDupe case-bug (`/^[A-E]\)/` ran AFTER toLowerCase → never matched prefixed options, silently disabling the duplicate-option gate) · regex assumed original case, no prefixed-duplicate test fixture · G2/G3 · strip → `/^\(?[a-e][\).]\s*/`. SN: mirror (SN gate likely has same bug).
- 2026-06-11 · D12 (PL) TEAS serve-path regression 2,313→155 approved **[RESOLVED]** · prior silver-tier promotion never persisted (NOT a mass-unapproval; gate only promotes) · G6 · ICA `deterministic-gate.mjs --course=TEAS --approve` → 2,313 restored; PCA `scripts/shipped-course-floor-probe.mjs` (DB-direct floor, wired into pages:deploy). SN: add own shipped-course floor probe if SN ships single-SKU courses.
- 2026-06-12 · **D16 (META — the recurring root cause behind D9/D14/D15)** the wired BIQ process (independent REV + student walk) kept getting SKIPPED across sessions even though it's "hard-wired." · **Root cause (5-why):** the only mechanical gate (`pre-release-check.js`) checked that a QA-walk FILE *exists + is fresh* — it validated **presence, not provenance**. The author could satisfy every check by *typing* a convincing walk; the gate couldn't tell a real independent-agent walk from a self-authored one, so "independence" was self-attested. Instructional wiring (CLAUDE.md, memory, this doc) is advisory + dropped at compaction; only **code** survives. · G4/G5 · **PCA (mechanism, not memory):** gate now verifies **PROVENANCE** — a real independent `Agent` invocation leaves `~/.claude/projects/<cwd-mangled>/<session>/subagents/agent-*.jsonl`; the gate hard-fails (`G4 QA Walk is SELF-AUTHORED`) unless such a transcript **postdates the newest src change** (also added the postdates-src `coversChanges` check SN lacked). A hand-typed `.md` no longer passes. Mirrored from PL where it was proven on its own release. The load-bearing fix because it cannot be skipped by forgetting.
- 2026-06-24 · **D44 (PL — activation breaker: new users create practice sessions but never reach an answerable question; ~16% of signups; lesson shared)** · **RCCA:** dashboard `layout.tsx` `trackSynced` did `window.location.replace` to sync a `?track=` mismatch → full reload → REMOUNTED the practice page mid-warm-up → lost the started-session state ("Setting up…" stall) → per-mount auto-start re-fired → duplicate phantom sessions, 0 answers. D25 fix exempted only `onboarding=1`; regular clep/dsst track-sync users (post-OAuth) still remounted. NOT serve/bank (each session had a valid question). · **ECA:** `startSession` lacked a synchronous re-entry guard (same-tick double-call risk). · **ICA:** layout skip reload on `/practice`; `startInFlightRef` guard in startSession. · **PCA (both repos):** activation E2E (first Q answerable + one-session-per-action) + a "no full-reload on /practice" check so a future reload can't silently remount the practice page.
- 2026-06-24 · **D43 (PL — ACT/SAT practice completion showed CLEP "Pass Probability" + "modeled on CLEP specifications"; lesson shared)** · **RCCA:** family-blind surfaces — content-disclaimer `disclaimerFor()` had no `act` case (ACT→CLEP default); practice/page.tsx session-complete drove pass-prob framing with no exam-family gate. D17 cross-family-scope-leak class. · **ECA:** scoreEstimate block already CLEP/DSST-gated; top metric + Pass-Score-Impact block were the leaks, both now `!isScoreNative`-gated; the shared content-disclaimer `act` case fixes all disclaimer surfaces. · **ICA:** add `case "act"` + gate the two pass-prob surfaces. · **PCA (both repos):** any score/readiness/disclaimer surface derives family from getCourseModule(course), never defaults to CLEP; QA-walk-each-family asserts ACT/SAT completion shows score-native, not CLEP pass-prob.
- 2026-06-23 · **D42 (PL — route "fix" claimed deployed but NEVER COMMITTED → still 404 on prod; lesson shared)** PL's `/practice/quickstart` redirect was created on disk + crash-doc'd as "deployed" but `git ls-files` shows it was never committed; with zero in-repo referrers it wasn't reliably in a prod build, so Codex's personas kept hitting the "page wandered off" 404. · **RCCA:** dev-server pass (serves from disk) substituted for a commit + prod-URL check — DoD=verified-LIVE miss (D5 lineage). · **ECA:** swept untracked src → `product-availability.ts` also untracked (live only via `--commit-dirty` builds); both committed so git matches prod. · **ICA:** committed + deployed; G6 curls the prod route. · **PCA (applies to BOTH repos):** pre-release scan that every `src/app/**/page.tsx` route is `git ls-files`-tracked (untracked = not shipped) + DoD reinforced: "deployed" requires a PROD-URL 200/redirect check, never a dev pass. Mirror the untracked-route scan into both `pre-release-check.js`.
